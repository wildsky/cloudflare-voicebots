...
async function testWeatherRequest() {
  const agentUrl = 'wss://cloudflare-voicebots.v5bqjpxcbw.workers.dev/agents/voicechat/test-room';
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (response.type === 'text-delta' || response.type === 'tool-call' || response.type === 'tool-result') {
