                ...
                <div className="flex-1 overflow-y-auto">
                    {conversations.map((conv) => (
                        <div
                            ...
                            className={`p-2 rounded cursor-pointer ${conv.id === currentConvId ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                                }`}
                        >
                            {conv.title}
                        </div>
            ...
            <div className="w-4/5 flex flex-col">
                ...
                <div className="flex flex-col flex-1 overflow-hidden">
                                        ...
                                        <div className={`flex gap-2 max-w-[85%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                                            {showAvatar && !isUser ? (
                                                <Avatar className="h-8 w-8 mt-1 flex-shrink-0">
                                                    <AvatarFallback className="bg-[#F48120] text-white">AI</AvatarFallback>
                                                </Avatar>
                                            ) : (
                                                ...
                                                {m.parts?.map((part, i) => {
                                                    if (part.type === "text") {
                                                        ...
                                                        return (
                                                            <div key={i}>
                                                                <Card
                                                                    className={`p-3 rounded-md ${isUser ? "bg-primary text-primary-foreground rounded-br-none" : "rounded-bl-none border-assistant-border"
                                                                        } ${isScheduled ? "border-accent/50" : ""} relative`}
                                                                >
                                                                    {isScheduled && <span className="absolute -top-3 -left-2 text-base">🕒</span>}
                                                                    <p className="text-sm whitespace-pre-wrap">{part.text.replace(/^scheduled message: /, "")}</p>
                                                                </Card>
                                                    ...
                                                    if (part.type === "tool-invocation") {
                                                        ...
                                                        if (
                                                            toolsRequiringConfirmation.includes(toolInvocation.toolName as keyof typeof tools) &&
                                                            ...
                                                            return (
                                                                <Card key={i} className="p-4 my-3 bg-secondary/30 border-secondary/50 rounded-md">
                                                                    ...
                                                                    <div className="flex gap-2 justify-end">
                                                                        <Button variant="outline" size="sm" onClick={() => addToolResult({ toolCallId, result: APPROVAL.NO })}>
                                                                            Reject
                                                                        </Button>
                                                                        <Button variant="default" size="sm" onClick={() => addToolResult({ toolCallId, result: APPROVAL.YES })}>
                                                                            Approve
                                                                        </Button>
                                                                    </div>
                                                                </Card>
                    ...
                    <div className="p-3 bg-input-background border-t">
                        <form onSubmit={(e) => handleAgentSubmit(e, { data: { annotations: { hello: "world" } } })}>
