...
async function simulateVoiceInteraction() {
  ...
  const agentUrl = 'wss://cloudflare-voicebots.v5bqjpxcbw.workers.dev/agents/twiliovoice/test-sim/websocket';
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
      console.log('Simulating user saying: "What\'s the weather in Seattle, WA?"');
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (data.toString().includes('weather') || data.toString().includes('Seattle')) {
        console.log('Raw response contains weather info:', data.toString().substring(0, 200));
      }
    }
  });
